...
func DecodeString(
	...
	for offset < len(data) {
		...
		case stringEscapeByte:
			decoded = append(decoded, literalEscapeGlyph)
		case dakutenMark, handakutenMark:
			if len(decoded) == 0 {
				return "", fmt.Errorf(
...
func EncodeString(value string, platform Platform) ([]byte, error) {
	...
	for _, r := range value {
		if r == literalEscapeGlyph {
			encoded = append(encoded, stringEscapeByte, stringEscapeByte)
			continue
		}
		if base, ok := removeVoiceMark(r, dakuten); ok {
			...
			encoded = append(encoded, index, stringEscapeByte, dakutenMark)
			continue
		}
		if base, ok := removeVoiceMark(r, handakuten); ok {
			...
			encoded = append(encoded, index, stringEscapeByte, handakutenMark)
			continue
		}
		index, ok := indices[r]
		if !ok {
	...
	return append(encoded, stringEscapeByte), nil
}
